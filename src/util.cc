...
FILE* open_file(string_view path, string_view mode)
	{
	if ( path.empty() )
		return 0;

	FILE* rval = fopen(path.data(), mode.data());
...
static bool can_read(string_view path)
	{
	return access(path.data(), R_OK) == 0;
	}

FILE* open_package(string& path, string_view mode)
	{
...
string without_bropath_component(string_view path)
	{
...
FILE* rotate_file(string_view name, RecordVal* rotate_info)
	{
	// Build file names.
	const int buflen = name.size() + 128;

	char newname[buflen], tmpname[buflen+4];

	snprintf(newname, buflen, "%s.%d.%.06f.tmp",
			name.data(), getpid(), network_time);
	newname[buflen-1] = '\0';
	...
	struct stat dummy;
	if ( link(name.data(), newname) < 0 || stat(newname, &dummy) < 0 )
		{
		reporter->Error("rotate_file: can't move %s to %s: %s", name.data(), newname, strerror(errno));
		fclose(newf);
		unlink(newname);
		unlink(tmpname);
		return 0;
	...
	if ( unlink(name.data()) < 0 || link(tmpname, name.data()) < 0 || unlink(tmpname) < 0 )
		{
		reporter->Error("rotate_file: can't move %s to %s: %s", tmpname, name.data(), strerror(errno));
